...
function parseDateTime(date, time) {
    ...
    return new Date(2000 + parseInt(dArray[2]), parseInt(dArray[1]), parseInt(dArray[0]), parseInt(tArray[0]), parseInt(tArray[1]), parseInt(tArray[2])).toJSON();
}
...
function DeviceListConfig(json) {
    ...
    wlandevices.map(device => parseWLanDevice(objects, device));

    const wlan5devices = sp.findEntries("template", "addmwlan5device", json);
    wlan5devices.map(device => parseWLanDevice(objects, device));
...
function parseWLanDevice(objects, device) {
