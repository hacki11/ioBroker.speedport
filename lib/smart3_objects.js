...
function initPhoneCalls() {
    ...
    value(objects, "calllists.inbound.count", "Number of Inbound Calls", "number", "", d =>
        countCalls("addtakencalls", d),
    );

    value(objects, "calllists.missed.json", "Missed Calls", "string", "", d => parseMissedCalls(d));
    value(objects, "calllists.missed.count", "Number of Missed Calls", "number", "", d =>
        countCalls("addmissedcalls", d),
    );

    value(objects, "calllists.outbound.json", "Outbound Calls", "string", "", d => parseOutboundCalls(d));
    value(objects, "calllists.outbound.count", "Number of Outbound Calls", "number", "", d =>
        countCalls("adddialedcalls", d),
    );
    ...
    return JSON.stringify(
        inboundCalls.map(call => {
            return {
                ...
                date: parseDateTime(
                    sp.findEntry("value", "dialedcalls_date", call),
                    sp.findEntry("value", "dialedcalls_time", call),
                ),
    ...
    return JSON.stringify(
        inboundCalls.map(call => {
            return {
                id: parseInt(sp.findEntry("value", "id", call)),
                date: parseDateTime(
                    sp.findEntry("value", "takencalls_date", call),
                    sp.findEntry("value", "takencalls_time", call),
                ),
    ...
    return JSON.stringify(
        missedCalls.map(call => {
            return {
                id: parseInt(sp.findEntry("value", "id", call)),
                date: parseDateTime(
                    sp.findEntry("value", "missedcalls_date", call),
                    sp.findEntry("value", "missedcalls_time", call),
                ),
...
Date.prototype.toISOLocaleString = function () {
    return `${this.getFullYear()}-${(this.getMonth() + 1).leftPad(2)}-${this.getDate().leftPad(
        2,
    )}T${this.getHours().leftPad(2)}:${this.getMinutes().leftPad(2)}:${this.getSeconds().leftPad(2)}`;
};
...
function parseWLanDevice(objects, device, channel) {
    ...
    valueDynamic(objects, `clients.${mac}.rssi`, "RSSI", "number", "dB", () =>
        parseInt(sp.findEntry("value", "mdevice_rssi", device)),
    );
}
...
function parseDevice(objects, device) {
    ...
    valueDynamic(
        objects,
        `clients.${mac}.downspeed`,
        "Downlink Speed",
        "number",
        "MBit/s",
        () => parseInt(sp.findEntry("value", "mdevice_downspeed", device)) / 1000 / 1000,
    );
    valueDynamic(
        objects,
        `clients.${mac}.upspeed`,
        ...
        "number",
        "MBit/s",
        () => parseInt(sp.findEntry("value", "mdevice_upspeed", device)) / 1000 / 1000,
    );
...
function initINetIP() {
    ...
    value(objects, "WAN.ipv4_dns_primary", "WAN IPv4 Primary DNS Server", "string", "", d =>
        parse("value", "dns_v4", d),
    );
    value(objects, "WAN.ipv4_dns_secondary", "WAN IPv4 Secondary DNS Server", "string", "", d =>
        parse("value", "sec_dns_v4", d),
    );
    ...
    value(objects, "WAN.ipv6_dns_primary", "WAN IPv6 Primary DNS Server", "string", "", d =>
        parse("value", "dns_v6", d),
    );
    value(objects, "WAN.ipv6_dns_secondary", "WAN IPv6 Secondary DNS Server", "string", "", d =>
        parse("value", "sec_dns_v6", d),
    );
...
function initSystemMessages() {
    ...
    value(objects, "DSL.downstream", "DSL Downstream", "number", "", d =>
        parseInt(parse("value", "dsl_downstream", d)),
    );
