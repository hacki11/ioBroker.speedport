...
module.exports = class EngineerMenu {
    ...
    async getDsl() {
        return this.get("/engineer/html/dsl_hidden_status.stm").then(response => this.parse(DslConfig, response.data));
    }

    async getInterfaceLan() {
        return this.get("/engineer/html/interfaces_hidden_lan.stm").then(response =>
            this.parse(InterfaceLanConfig, response.data),
        );
    }

    async getInterfaceWan() {
        return this.get("/engineer/html/interfaces_hidden_wan.stm").then(response =>
            this.parse(InterfaceWanConfig, response.data),
        );
    }

    async getMemCpuUtilization() {
        return await this.get("/engineer/html/mem_cpu_utilization.stm").then(response =>
            this.parse(MemCpuUtilizationConfig, response.data),
        );
    }

    async getModuleVersions() {
        return await this.get("/engineer/html/module_versions.stm").then(response =>
            this.parse(ModuleVersionsConfig, response.data),
        );
    }
    ...
    parse(configObjects, text) {
        for (const config of configObjects) {
            config.value = config.parser(text);
        }
