...
module.exports = class Smart3 {
    ...
    async login() {
        this.httoken = await sputils.getHttoken(this.axiosPlain, this.host);
    ...
    async getSystemMessages() {
        return sputils.getHttoken(this.axiosPlain, this.host + "/html/content/config/system_info.html", {params: {lang: "DE"}})
            .then(httoken => this.get("/data/SystemMessages.json", httoken))
            .then(response => this.parse(SystemMessagesConfig, response.data));
    }

    async get(url, httoken) {
        return this.axiosInstance.get(this.host + url, this.getParams(httoken));
    }
