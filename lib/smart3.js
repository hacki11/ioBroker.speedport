...
module.exports = class Smart3 {
    ...
    async getDeviceList() {
        return this.get("/html/content/network/devices.html", "/data/DeviceList.json")
            .then(response => this.parse(DeviceListConfig(response.data), response.data));
    }

    async getPhoneCalls() {
        return this.get("/html/content/phone/phone_call_list.html", "/data/PhoneCalls.json")
            .then(response => this.parse(PhoneCallsConfig, response.data));
    }
