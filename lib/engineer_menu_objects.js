...
function initInterfaceLan() {
    ...
    for (let i = 1; i < 5; i++) {
        value(objects, `LAN.LAN${i}.link_speed`, `LAN${i} Link Speed`, "number", "Mbits", d => parseInt(parse(`inte_face_speed\\[${i - 1}\\] = (\\d+);`, d)));
        value(objects, `LAN.LAN${i}.tx_packets`, `LAN${i} TX Packets`, "number", "", d => parseInt(parse(`inte_face_tx\\[${i - 1}\\]\\[0\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.tx_bytes`, `LAN${i} TX Bytes`, "number", "byte", d => parseInt(parse(`inte_face_tx\\[${i - 1}\\]\\[1\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.tx_errors`, `LAN${i} TX Errors`, "number", "", d => parseInt(parse(`inte_face_tx\\[${i - 1}\\]\\[2\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.tx_dropped`, `LAN${i} TX Dropped`, "number", "", d => parseInt(parse(`inte_face_tx\\[${i - 1}\\]\\[3\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.rx_packets`, `LAN${i} RX Packets`, "number", "", d => parseInt(parse(`inte_face_rx\\[${i - 1}\\]\\[0\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.rx_bytes`, `LAN${i} RX Bytes`, "number", "byte", d => parseInt(parse(`inte_face_rx\\[${i - 1}\\]\\[1\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.rx_errors`, `LAN${i} RX Errors`, "number", "", d => parseInt(parse(`inte_face_rx\\[${i - 1}\\]\\[2\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.rx_dropped`, `LAN${i} RX Dropped`, "number", "", d => parseInt(parse(`inte_face_rx\\[${i - 1}\\]\\[3\\] = ([-]?\\d+);`, d)));
        value(objects, `LAN.LAN${i}.collision`, `LAN${i} Collisions`, "number", "", d => parseInt(parse(`inte_face_collisions\\[${i - 1}\\] = ([-]?\\d+);`, d)));
    }
